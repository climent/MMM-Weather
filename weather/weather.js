...
Module.register("weather", {
	...
	notificationReceived (notification, payload, sender) {
		if (notification === "CALENDAR_EVENTS") {
			...
			this.indoorTemperature = payload;
			this.updateDom(300);
		} else if (notification === "INDOOR_HUMIDITY") {
			this.indoorHumidity = payload;
			this.updateDom(300);
			...
			function (value, type, valueUnit) {
				...
				if (type === "temperature") {
					if (value === null || value === undefined) {
						formattedValue = "";
					} else if (this.config.showBothTempUnits) {
						const celsius = `${this.roundValue(WeatherUtils.convertTemp(value, "metric"))}°C`;
						const fahrenheit = `${this.roundValue(WeatherUtils.convertTemp(value, "imperial"))}°F`;
						if (this.config.tempUnits === "imperial") {
							formattedValue = `${fahrenheit} / ${celsius}`;
						} else {
							formattedValue = `${celsius} / ${fahrenheit}`;
						}
					} else {
